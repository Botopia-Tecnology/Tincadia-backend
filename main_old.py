...
async def startup_event():
    print("­ƒÜÇ [Startup] Iniciando microservicio Model-ms...")
    try:
        ...
        if model is not None:
            print(f"Ô£à [Startup] Modelo precargado exitosamente. Clases: {len(labels)}")
        else:
            print("ÔØî [Startup] Error: El modelo no se pudo precargar (es None)")
    except Exception as e:
        print(f"ÔØî [Startup Error] Fallo cr├¡tico cargando modelo: {e}")
        traceback.print_exc()
...
async def connect(sid, environ):
    log(f"­ƒöî [Socket.IO] Intento de conexi├│n: {sid}")
    try:
        # Obtener predictor compartido (ya cargado en startup)
        base_predictor = LSCEngine.get_predictor()
        
        if base_predictor is None:
            log(f"ÔØî [Socket.IO Error] El predictor NO est├í listo. Intentando cargar...")
            base_predictor = LSCEngine.get_predictor() # Reintento carga
            if base_predictor is None:
                log(f"ÔØî [Socket.IO Error] Fallo cr├¡tico: modelo inaccesible. Rechazando {sid}")
                return False
        ...
        log(f"Ô£à [Socket.IO] Conexi├│n aceptada para {sid}")
    except Exception as e:
        if LOGS_ENABLED:
            print(f"ÔØî [Socket.IO Error] Excepci├│n fatal en connect para {sid}: {e}")
            traceback.print_exc()
        return False
