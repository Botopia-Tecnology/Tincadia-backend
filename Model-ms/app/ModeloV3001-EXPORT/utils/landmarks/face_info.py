...
# CANT_LANDMARKS_FACE = 478 * 3
CANT_LANDMARKS_FACE = 468 * 3


class FaceLandmark(enum.IntEnum):
    """The 468 face landmarks."""

    @classmethod
    def has_value(cls, value):
        return value in range(int(CANT_LANDMARKS_FACE / 3))


class FaceInfo(LandmarkInfo):
    def __init__(
        self,
        results: Union[any, CoordSignal],
    ):
        if isinstance(results, CoordSignal):
            landmarks = results.face
        else:
            # MediaPipe results object or similar
            landmarks = getattr(results, 'face_landmarks', None)

        if landmarks is None:
            landmarks = [Coords.empty()] * int(CANT_LANDMARKS_FACE / 3)
        ...
        super().__init__(
            landmark=landmarks,
            points=FaceLandmark,
            connections=self._face_connections(),
            name="face",
            has_visibility=False,
        )

    def _face_connections(self):
        # fmt: off
