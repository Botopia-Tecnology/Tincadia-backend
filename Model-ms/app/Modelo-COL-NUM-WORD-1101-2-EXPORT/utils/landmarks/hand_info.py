...
class HandInfo(LandmarkInfo):
    def __init__(
        self,
        results: Union[any, CoordSignal],
        hand: Hand = Hand.RIGHT,
    ):
        if isinstance(results, CoordSignal):
            landmarks = results.rightHand if hand == Hand.RIGHT else results.leftHand
        else:
            # MediaPipe results object or similar
            landmarks = getattr(results, 'right_hand_landmarks' if hand == Hand.RIGHT else 'left_hand_landmarks', None)

        if landmarks is None:
            landmarks = [CoordsWithouthVisibility.empty()] * int(CANT_LANDMARKS_HAND / 3)
        elif not isinstance(results, CoordSignal) and hasattr(landmarks, 'landmark'):
